...
class NormLayer(ABSLayer, Neuron):
    def __init__(self, d_model: int):
        self.weights = mo.ones(d_model)
        self.bias = mo.zeros(d_model)
        self.epsilon = 1e-5
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        self.mean = mo.mean(x, axis=-1, keepdims=True)
        self.variance = mo.var(x, axis=-1, keepdims=True) + self.epsilon
    ...
    def backward(self, incoming_grad: np.ndarray) -> np.ndarray:
        self.d_bias = mo.sum(incoming_grad, axis=(0, 1))
        self.d_weights = mo.sum(incoming_grad * self.x_hat, axis=(0, 1))

        dx_hat = incoming_grad * self.weights
        N = self.x_hat.shape[-1]
        std_inv = 1.0 / mo.sqrt(self.variance)
        
        sum_dxhat = mo.sum(dx_hat, axis=-1, keepdims=True)
        sum_dxhat_xhat = mo.sum(dx_hat * self.x_hat, axis=-1, keepdims=True)
