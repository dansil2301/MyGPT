...
class Linear(ABSLayer, Neuron):
    def __init__(self, in_feature: int, out_feature: int):
        self.weights = mo.he_normal_init(in_feature, out_feature)
        self.bias = mo.zeros(out_feature)
    ...
    def backward(self, incoming_grad: np.ndarray) -> np.ndarray:
        self.d_weights = mo.sum(self.input.transpose(0, 2, 1) @ incoming_grad, axis=0)
